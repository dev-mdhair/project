 ...
 *
 * <code>
 * $geoip = GeoIP::getInstance('/path/to/geoipdb.dat', GeoIP::SHARED_MEMORY);
 * </code>
 *
 ...
 * @package  NetGeoIP
 * @author   Jim Winstead <jimw@apache.org> (original Maxmind PHP API)
 * @author   Hans Lellelid <hans@xmpl.org> (PEAR port)
 * @author   Dinis Lage <dinislage@gmail.com> (Yii port)
 * @license  LGPL http://www.gnu.org/licenses/lgpl.txt
 * @link     http://pear.php.net/package/GeoIP
 */
class GeoIP {
	 ...
	 */
	const ERR_INVALID_IP = 218624992; // crc32('GeoIP::ERR_INVALID_IP')
	 ...
	 * $geoip = GeoIP::getInstance('C:\mydb.dat', GeoIP::MEMORY_CACHE);
	 * ....
	 *
	 * // later in code, request instance with no flags specified.
	 * $geoip = GeoIP::getInstance('C:\mydb.dat');
	 *
	...
	public static function getInstance($filename = null, $flags = null) {
		if (!isset(self::$instances[$filename])) {
			self::$instances[$filename] = new GeoIP($filename, $flags);
		}
	 ...
	 * @return void
	 *
	 * @throws CException if unable to open specified file or shared memory.
	 */
		...
		if ($this->flags & self::SHARED_MEMORY) {
			...
			if ($this->shmid === false) {
				...
				if ($this->shmid === false) { // should never be false as loadSharedMemory() will throw Exc if cannot create
					throw new CException("Unable to open shared memory at key: " . dechex(self::SHM_KEY));
				}
			}
		} else {
	 ...
	 *
	 * @throws CException  - if IP address is invalid.
	...
	protected function lookupCountryId($addr) {
		$ipnum = ip2long($addr);
		if ($ipnum === false) {
			throw new CException("Invalid IP address: " . var_export($addr, true), self::ERR_INVALID_IP);
		}
		if ($this->databaseType !== self::COUNTRY_EDITION) {
			throw new CException("Invalid database type; lookupCountry*() methods expect Country database.");
		}
	 ...
	 *
	 * @throws CException (see lookupCountryId())
	 * @see lookupCountryId()
	 */
	 ...
	 * @throws CException - if fseek() fails on the file or no results after traversing the database (indicating corrupt db).
	 */
	protected function seekCountry($ipnum) {
		...
		for ($depth = 31; $depth >= 0; --$depth) {
			if ($this->flags & self::MEMORY_CACHE) {
				...
				if (fseek($this->filehandle, 2 * $this->recordLength * $offset, SEEK_SET) !== 0) {
					throw new CException("fseek failed");
				}
		...
		throw new CException("Error traversing database - perhaps it is corrupt?");
	}
	...
	public function lookupOrg($addr) {
		...
		if ($this->databaseType !== self::ORG_EDITION) {
			throw new CException("Invalid database type; lookupOrg() method expects Org/ISP database.", self::ERR_DB_FORMAT);
		}
	...
	public function lookupRegion($addr) {
		...
		if ($this->databaseType !== self::REGION_EDITION_REV0 && $this->databaseType !== self::REGION_EDITION_REV1) {
			throw new CException("Invalid database type; lookupRegion() method expects Region database.", self::ERR_DB_FORMAT);
		}
	 ...
	 * @param string $addr IP address (hostname not allowed).
	 *
	 * @return GeoIP_Location The full location record.
	 *
	 * @throws CException - if IP address is invalid.
	 */
	public function lookupLocation($addr, $default_country = null) {
		include_once 'GeoIP/Location.php';
		include_once 'GeoIP/CRN.php';
		$ipnum = ip2long($addr);
		if ($ipnum === false) {
			throw new CException("Invalid IP address: " . var_export($addr, true), self::ERR_INVALID_IP);
		}
		if ($this->databaseType !== self::CITY_EDITION_REV0 && $this->databaseType !== self::CITY_EDITION_REV1) {
			throw new CException("Invalid database type; lookupLocation() method expects City database.");
		}
		return $this->getRecord($ipnum, $default_country);
	}
	...
	/**
	 * Seek and populate GeoIP_Location object for converted IP addr.
	 * Note: this
	 *
	 * @param int $ipnum Converted IP address.
	 *
	 ...
	 */
	protected function getRecord($ipnum, $default_country = NULL) {
		$seek_country = $this->seekCountry($ipnum);
		if ($seek_country == $this->databaseSegments) {
			$record = new GeoIP_Location();
			$record->countryCode = $default_country;
			return $record;
		}
