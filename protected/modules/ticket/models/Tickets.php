...
class Tickets extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('total', 'length', 'max'=>128),
			array('text, create_at', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, text, status, total, create_at, users_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'notificationsUsers' => array(self::HAS_MANY, 'NotificationsUser', 'tickets_id'),
			'orders' => array(self::HAS_MANY, 'Orders', 'tickets_id'),
			'ticketItems' => array(self::HAS_MANY, 'TicketItem', 'tickets_id'),
